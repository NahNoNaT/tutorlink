...
export default function StudentRequestSuccess() {
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-16 text-center">
      ...
      <h1 className="text-2xl font-bold text-white">Đã nhận yêu cầu</h1>
      <p className="mt-2 text-slate-400">Đơn học của bạn đã được ghi nhận và đang chờ gia sư phù hợp nhận.</p>
      <div className="mt-6 inline-flex gap-2">
        ...
        {id && <Link href={`/chat/${id}`} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-slate-100">Mở Chat</Link>}
      </div>
    </div>
  )
}
