...
export default function StudentBookingsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!user) { setMsg('Vui lòng đăng nhập'); setLoading(false); return }
    ...
    <div className="mx-auto max-w-3xl px-4">
      ...
      {rows.map((b) => (
        <div key={b.id}>
          <Link
            href={`/bookings/${b.id}`}
            className="mb-2 block rounded-xl border border-white/10 bg-white/5 p-4 text-slate-200 hover:bg-white/10"
          >
            <div className="flex items-center justify-between">
              <div className="font-semibold">Booking #{b.id}</div>
              <div className="text-sm text-slate-400">{new Date(b.start_time).toLocaleDateString()}</div>
            </div>
          ...
          {b.request_id && (
            <div className="mb-3">
              <a
                className="inline-block rounded-xl border border-white/10 bg-white/5 px-3 py-1 text-sm text-slate-100 hover:bg-white/10"
                href={`/chat/${b.request_id}`}
              >
                Chat
              </a>
            </div>
      ...
      {rows.length === 0 && <div className="text-slate-400">Bạn chưa có booking nào.</div>}
